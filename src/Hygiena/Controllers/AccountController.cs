    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            try
            {
                ...
                if (result.IsLockedOut)
                    error = "User account locked out.";
                ModelState.AddModelError(string.Empty, $"Invalid login attempt.{error}");
                return View(model);
            }
            catch (Exception ex)
            {
                _logger.LogCritical(1, ex, "Login  Failed ");
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            try
            {
                ...
                var adminRoles =
                    _dataService.Get<IdentityRole>(
                        x => !x.Name.Contains("hygiena") && !x.Name.Contains("unitOperatorEnabled"));
                ...
                if (result.Succeeded)
                {
                    _dataService.CreateNewAccount(model, user);
            ...
            {
                _logger.LogCritical(1, ex, $"Registeration Count Failed ");
                var userDeleted = _userManager.DeleteAsync(user).Result;
        ...
        public async Task<IActionResult> LogOff()
        {
            ...
            return RedirectToAction("MainView", "Navigation");
            //    return RedirectToAction(nameof(NavigationController.MainView), "MainView");
        }
        ...
        private IActionResult RedirectToLocal(string returnUrl)
        {
            return Url.IsLocalUrl(returnUrl)
                ...
                : RedirectToAction("MainView", "Navigation");
        }
