...
// All available plans that have NOT been added to a unit (i.e. difference of allPlans and assignedPlansPerUnit)
        ...
        $scope.movePlansFromAvailableToAssigned = function() {
            var allPlansDataGrid = $("#allPlansPerUnitGrid").dxDataGrid("instance");
            var arrSelectedAvailPlans = allPlansDataGrid.getSelectedRowsData();
            if (arrSelectedAvailPlans !== undefined && arrSelectedAvailPlans.length > 0) {
                for (i = 0; i < arrSelectedAvailPlans.length; i++) {
        ...
        $scope.movePlansFromAssignedToAvailable = function() {
            var assignedPlansDataGrid = $("#assignedPlansGrid").dxDataGrid("instance");
            var arrSelectedAssignedPlans = assignedPlansDataGrid.getSelectedRowsData();
            if (arrSelectedAssignedPlans !== undefined && arrSelectedAssignedPlans.length > 0) {
                for (i = 0; i < arrSelectedAssignedPlans.length; i++) {
        ...
        function populateSelectedUnitDetail(serialNo) {
            ...
            for (i = 0; i < $scope.units.length; i++) {
                if ($scope.units[i].serialNo === serialNo) {
                    ...
                    for (j = 0; j < $scope.selectedUnit.assignedPlans.length; j++) {
        ...
        function getArrayDiff(arrSuperSet, arrSubSet) {
            ...
            for (i = 0; i < arrSuperSet.length; i++) {
                var result = $.grep(arrSubSet, function(e) { return e.id === arrSuperSet[i].id; });
